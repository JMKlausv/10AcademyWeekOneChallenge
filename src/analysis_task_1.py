...
def make_analysis_on_slack(data_path):
    ...
    channel_data = df.groupby('channel').agg({'msg_content': 'count', 'reply_count': 'sum', 'reactions_count': 'sum'}).reset_index()

    # Plot
    plt.figure(figsize=(10, 8))
    scatter_plot = sns.scatterplot(x='msg_content', y='reply_count', hue='channel', data=channel_data)
    plt.show()
    # Fraction of Messages Replied Within First 5 Minutes:
    print("^^^^^^^^^^^^@@@@@@@ Fraction of Messages Replied Within First 5 Minutes @@@@@@@@@^^^^^^^^^^^^^^^")
    ...
    messages_replied_within_5mins = df[df['time_diff'] <= pd.to_timedelta('300 seconds')].groupby('channel')['msg_content'].count()
   ...
   # Calculate first reply time for each channel
    ...
    print("^^^^^^^^^^^^@@@@@@@ 2D Scatter Plot with Time Difference and Time of the Day: @@@@@@@@@^^^^^^^^^^^^^^^")
    ...
    df['time_difference'] = (df['tm_thread_end'] - df['msg_sent_time']).dt.total_seconds()

    # Plot
    plt.figure(figsize=(10, 8))
    scatter_plot_time = sns.scatterplot(x='time_difference', y=df['msg_sent_time'].dt.hour, hue='channel', data=df)
    plt.show()
